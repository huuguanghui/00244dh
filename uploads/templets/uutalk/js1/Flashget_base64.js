...
function convertFgURL(url,id){
	try{
		FlashgetDown(FlashgetEncode_js(this.fUrl,id),id);
	}catch(e){
		//alert(e.message);
		location.href = this.fUrl;
	}

}

function Flashget_SetHref_js(obj,uid){
	obj.href = FlashgetEncode_js(this.fUrl,uid) ;
}

function FlashgetEncode_js(t_url,uid) {
	var prefix = "Flashget://";
	var FlashgetURL =  prefix + fg_base64encode(fg_utf16to8("[FLASHGET]" +t_url + "[FLASHGET]"))+"&"+uid;
	return FlashgetURL;
}
